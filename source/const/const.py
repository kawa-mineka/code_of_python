...
#チャンネルの定数定義(主にサウンド再生時のチャンネル指定に使います)
CH0 = 0 #チャンネル0
CH1 = 1 #チャンネル1
CH2 = 2 #チャンネル2
CH3 = 3 #チャンネル3
...
PREDEF_BGANIME_ON  = 1
#BG書き換え用オブジェクト(事前に書き換え指定のマップチップを探して座標を登録記録していくタイプ)マーカー関連の定数定義
BG_OBJ_PREDEF_DUMMY     = 0                         #ダミーマーカー
BG_OBJ_PREDEF_MASTER    = 64                        #マーカー「’0」のアスキーコード 固定しておいてくださいな
BG_OBJ_PREDEF_ELEVATOR0 = (16 // 8) * 32 + (  0 // 8) #4面高層ビル用エレベーターその0  アスキーコードは(16 / 8) * 32 + ( 0 / 8)  #マップチップ  x0y16 ショットアイテム 公式は(y/8)*32 + (x/8)となります
BG_OBJ_PREDEF_ELEVATOR1 = (16 // 8) * 32 + (  8 // 8) #4面高層ビル用エレベーターその1  アスキーコードは(16 / 8) * 32 + ( 8 / 8)  #マップチップ  x8y16 ショットアイテム 公式は(y/8)*32 + (x/8)となります
BG_OBJ_PREDEF_ELEVATOR2 = (16 // 8) * 32 + ( 16 // 8) #4面高層ビル用エレベーターその2  アスキーコードは(16 / 8) * 32 + (16 / 8)  #マップチップ x16y16 ショットアイテム 公式は(y/8)*32 + (x/8)となります
BG_OBJ_PREDEF_ELEVATOR3 = (16 // 8) * 32 + ( 24 // 8) #4面高層ビル用エレベーターその3  アスキーコードは(16 / 8) * 32 + (24 / 8)  #マップチップ x24y16 ショットアイテム 公式は(y/8)*32 + (x/8)となります
BG_OBJ_PREDEF_ELEVATOR4 = (16 // 8) * 32 + ( 32 // 8) #4面高層ビル用エレベーターその4  アスキーコードは(16 / 8) * 32 + (32 / 8)  #マップチップ x32y16 ショットアイテム 公式は(y/8)*32 + (x/8)となります
BG_OBJ_PREDEF_ELEVATOR5 = (16 // 8) * 32 + ( 40 // 8) #4面高層ビル用エレベーターその5  アスキーコードは(16 / 8) * 32 + (40 / 8)  #マップチップ x40y16 ショットアイテム 公式は(y/8)*32 + (x/8)となります
...
#縦スクロール可能な面での縦画面数
SINGLE_SCREEN = 1          #縦は常に1画面で縦スクロールはしない
DUAL_SCREEN   = 2          #縦は2画面分あり、任意にスクロールする
TRIPLE_SCREEN = 3          #縦は3画面分あり、任意にスクロールする

#背景BGスクロールで敵をどのように出現させるかどうかのバリエーション
BG_ENEMY_NONE   =   0      #BGマップでは敵は出現させない
