...
#ウィンドウのidの定数定義2025 02/23より別クラスにて列挙型を使用し自動で任意の数値を定義するようにしました(中に入る数字はどんな数値でも良い為です)
...
#2025 02/23より別クラスにて列挙型を使用し自動で任意の数値を定義するようにしました
class Window_id(IntEnum):
    NO_MENU                        = auto() #ダミー用 ノーメニュー
    MAIN_MENU                      = auto() #タイトル画面からのメインメニュー
    SELECT_STAGE_MENU              = auto() #ステージ選択メニュー
    SELECT_LOOP_MENU               = auto() #ループ数選択メニュー
    BOSS_MODE_MENU                 = auto() #ボスモードオンオフメニュー
    HITBOX_MENU                    = auto() #ヒットボックス(当たり判定)表示オンオフメニュー(ボスのみ)
    SELECT_DIFFICULTY              = auto() #難易度選択メニュー
    SELECT_REPLAY_SLOT             = auto() #どのスロットのリプレイデータをロードするのかのメニュー
    GAME_OVER_RETURN               = auto() #ゲームオーバーから戻る時のメニュー
    GAME_OVER_RETURN_NO_SAVE       = auto() #ゲームオーバーから戻る時のメニュー(リプレイデータのセーブは無し)
    SELECT_FILE_SLOT               = auto() #リプレイデータをセーブするスロットを選択するメニュー
    SCORE_BOARD                    = auto() #スコアボードウィンドウ
    INPUT_YOUR_NAME                = auto() #名前入力ウィンドウ
    CONFIG                         = auto() #各種設定ウィンドウ(CONFIGウィンドウ)
    CONFIG_GRAPHICS                = auto() #グラフイック設定ウィンドウ
    CONFIG_SOUND                   = auto() #サウンド設定ウィンドウ
    CONFIG_CONTROL                 = auto() #コントロール設定ウィンドウ
    CONFIG_SETTING                 = auto() #ゲーム設定ウィンドウ
    STATUS                         = auto() #ステータスウィンドウ
    DUMMY_TEST                     = auto() #ダミーテスト用ウィンドウ
    ACHIEVEMENT_LIST               = auto() #実績(アチーブメント)リストウィンドウ
    MEDAL_LIST                     = auto() #メダルウィンドウリスト
    EQUIPMENT                      = auto() #装備ウィンドウ
    ITEM_LIST                      = auto() #アイテムリストウィンドウ
    EXIT                           = auto() #ゲーム終了(退出)ウィンドウ
    PAUSE_MENU                     = auto() #ポーズメニュー
    RETURN_TITLE                   = auto() #リターンタイトルウィンドウ
    SLOT                           = auto() #スロットウィンドウ
    SELECT_SHIP                    = auto() #自機選択ウィンドウ
    INITIALIZE                     = auto() #システムデータ初期化選択ウィンドウ
    SELECT_YES_NO                  = auto() #イエスノー選択ウィンドウ
    PRINT_INIT_SCORE               = auto() #スコアデータ初期化メッセージ表示ウィンドウ
    PRINT_INIT_NAME                = auto() #ネームデータ初期化メッセージ表示ウィンドウ
    PRINT_INIT_SHIP                = auto() #シップデータ初期化メッセージ表示ウィンドウ
    PRINT_INIT_MEDAL               = auto() #メダルデータ初期化メッセージ表示ウィンドウ
    PRINT_INIT_ACHIEVEMENT         = auto() #アチーブメントデータ初期化メッセージ表示ウィンドウ
    PRINT_INIT_ALL                 = auto() #オールデータ初期化メッセージ表示ウィンドウ
    MEDAL_ACQUISITION_REPORT       = auto() #メダル取得報告ウィンドウ
    ACHIEVEMENT_ACQUISITION_REPORT = auto() #実績(アチーブメント)取得報告ウィンドウ
    JOYPAD_ASSIGN                  = auto() #ジョイパッドボタン割り当て設定ウィンドウ
    TITLE_TEXT                     = auto() #タイトルで表示されるテキスト群ウィンドウ(フレームレス)(外枠は表示しない)
    TITLE_STORY_TEXT               = auto() #タイトルで表示されるストーリー(フレームレス)(外枠は表示しない)(上方向に自動スクロールします)

#ウィンドウのid_subの定数定義 windowクラスの window[i].window_id_subに入ります
    SUB_NORMAL_MENU                = auto() #通常の選択メニュー
    SUB_YES_NO_MENU                = auto() #「はい」「いいえ」の2択式メニュー
    SUB_ON_OFF_MENU                = auto() #「ON」「 OFF」の2択式メニュー        (フラグオン時は選択されている物がハイライトされた色となります)
    SUB_SELECT_NUM_MENU            = auto() #数値を横方向の操作で増減させて決めるメニュー
    SUB_TOGGLE_MENU                = auto() #押すことでオンオフを切り替えることが出来るトグルスイッチメニュー
    SUB_FULL_4WAY_MENU             = auto() #4方向入力による自由なデザインでのメニュー
    SUB_RIGHT_LEFT_PAGE_MENU       = auto() #左右の頁送りで切り替えるメニュー(スコアボードとかで使用)
    SUB_MULTI_SELECT_MENU          = auto() #通常の選択メニューと基本は同じだがあらかじめ選択された物がハイライトされた色で表示される
    SUB_SWITCH_TEXT_MENU           = auto() #上下操作でカーソルが上下し左右でそれぞれの項目に対応したテキストが切り替わり表示されるメニュータイプ
