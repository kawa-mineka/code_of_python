...
from update.update_bg import * #BGタイルマップを読み出したり書き換えたいのでインポート
...
class update_system:
    ...
    def read_data_num(self,x,y,tm,digit):      #x,yは1の位の座標です,tmはtilemapの数値,digitは桁数です
        ...
        for i in range(digit):
            num += (update_bg.get_chrcode_tilemap(self,tm,x-i,y) - 16) * a
            a = a * 10
        return(num)
    ...
    def write_data_num(self,x,y,tm,digit,num): #x,yは1の位の座標です,tmはtilemapの数値,digitは桁数 numは書き込む数値です(整数を推奨)
        ...
        for i in range(digit):
            n = num % a * 10 // a
            update_bg.set_chrcode_tilemap(self,tm,x-i,y,n + 16)
    ...
    def load_data(self):
        ...
        self.game_difficulty = update_bg.get_chrcode_tilemap(self,0,0,120) - 16 #数字の[0]はアスキーコード16番なので16引いて数値としての0にしてやります
        # print(self.game_difficulty)
        self.stage_number    = update_bg.get_chrcode_tilemap(self,0,0,121) - 16
        # print(self.stage_number)
        self.stage_loop      = update_bg.get_chrcode_tilemap(self,0,0,122) - 16
        ...
        #総ゲームプレイ時間(秒)を計算する
        sec_1  = update_bg.get_chrcode_tilemap(self,0,9,5) - 16 #秒の  1の位取得
        sec_10 = update_bg.get_chrcode_tilemap(self,0,8,5) - 16 #秒の  10の位取得
        min_1  = update_bg.get_chrcode_tilemap(self,0,6,5) - 16 #分の  1の位取得
        min_10 = update_bg.get_chrcode_tilemap(self,0,5,5) - 16 #分の  10の位取得
        hour_1 = update_bg.get_chrcode_tilemap(self,0,3,5) - 16 #時の   1の位取得
        hour_10 = update_bg.get_chrcode_tilemap(self,0,2,5) - 16 #時の   10の位取得
        hour_100 = update_bg.get_chrcode_tilemap(self,0,1,5) - 16 #時の   100の位取得
        hour_1000 = update_bg.get_chrcode_tilemap(self,0,0,5) - 16 #時の   1000の位取得
        ...
        #総開発テスト時間(分)を計算する
        min_1  = update_bg.get_chrcode_tilemap(self,0,7,3) - 16 #分の  1の位取得
        min_10 = update_bg.get_chrcode_tilemap(self,0,6,3) - 16 #分の  10の位取得
        hour_1 = update_bg.get_chrcode_tilemap(self,0,4,3) - 16 #時の   1の位取得
        hour_10 = update_bg.get_chrcode_tilemap(self,0,3,3) - 16 #時の   10の位取得
        hour_100 = update_bg.get_chrcode_tilemap(self,0,2,3) - 16 #時の   100の位取得
        hour_1000 = update_bg.get_chrcode_tilemap(self,0,1,3) - 16 #時の   1000の位取得
        hour_10000 = update_bg.get_chrcode_tilemap(self,0,0,3) - 16 #時の   10000の位取得
        m = min_10 * 10 + min_1
        ...
        self.debug_menu_status             = update_bg.get_chrcode_tilemap(self,0,0,126) - 16 #数字の[0]はアスキーコード16番なので16引いて数値としての0にしてやります
        
        #boss_collision_rect_display_flag        ボス用の当たり判定確認の為の矩形表示フラグ(デバッグ時に1にします)
        self.boss_collision_rect_display_flag = update_bg.get_chrcode_tilemap(self,0,0,127) - 16
        #bg_collision_Judgment_flag            背景の障害物との衝突判定を行うかどうかのフラグ
                                            #0=背景の障害物との当たり判定をしない 1=行う
        self.bg_collision_Judgment_flag      = update_bg.get_chrcode_tilemap(self,0,0,128) - 16
        #boss_test_mode                      ボス戦闘のみのテストモード 
                                            #0=オフ 1=オン scroll_countを増やさない→マップスクロールしないので敵が発生しません
                                            #イベントリストもボス専用の物が読み込まれます
        self.boss_test_mode                = update_bg.get_chrcode_tilemap(self,0,0,129) - 16
        #no_enemy_mode                       マップチップによる敵の発生を行わないモードのフラグですです(地上の敵が出ない！)2021 03/07現在機能してない模様
                                            #0=マップスクロールによって敵が発生します
                                            #1=                    発生しません        
        self.no_enemy_mode                 = update_bg.get_chrcode_tilemap(self,0,0,130) - 16
        #god_mode_status                    #ゴッドモードのステータス
                                            #0=ゴッドモードオフ 1=ゴッドモードオン
        self.god_mode_status               = update_bg.get_chrcode_tilemap(self,0,0,131) - 16
        #fullscreen_mode                    #フルスクリーンでの起動モード
                                            #0=ウィンドウモードでの起動 1=フルスクリーンモードでの起動
        self.fullscreen_mode               = update_bg.get_chrcode_tilemap(self,0,0,132) - 16
        #ctrl_type                          #コントロールパッドのタイプ
                                            #0~5
        self.ctrl_type                     = update_bg.get_chrcode_tilemap(self,0,0,133) - 16
        ...
        #language                           #選択言語
                                            #0=英語 1=日本語
        self.language                      = update_bg.get_chrcode_tilemap(self,0,0,136) - 16
        
        #メダル所有フラグの読み込み
        for i in range(10):
            self.medal_list[i] = update_bg.get_chrcode_tilemap(self,0,0,210 + i) - 16
        
        #各機体のスロット装備済みメダルデータの読み込み
        for i in range(LOOK_AT_LOGO): #iは0=J_pythonからLOOK_AT_LOGOまで変化
            for j in range(6):        #jはスロット0からスロット6まで変化
                self.ship_equip_slot_list[i][j] = update_bg.get_chrcode_tilemap(self,0,23 + j,11 + i) - 16
        ...
        for i in range(GAME_INSAME+1): #iは0からGAME_INSAME+1(6)まで変化する(難易度)
            ...
            for j in range(11):      #jは0から11まで変化する(順位)
                ...
                for k in range(8):   #kは0から8まで変化する(8文字だよ)
                    st = chr(update_bg.get_chrcode_tilemap(self,tm,3 + k,5 + j + (i * 16))+32)  #sysytemデータで使用しているフォントはアスキーコードと実際の文字列に32の差が出るので注意(フォントマップの横幅が32キャラだから)
                    namestr = namestr + st
                ...
                for l in range(6): #機体に装着されたメダル装備IDの読み込み
                    self.score_board[i][j][LIST_SCORE_BOARD_SHIP_SLOT0 + l] = update_bg.get_chrcode_tilemap(self,tm,41 + l,5 + j + (i * 16)) - 16
        
        #名前の読み込み
        namestr = ""
        for i in range(8):   #kは0から8まで変化する(8文字だよ)
            st = chr(update_bg.get_chrcode_tilemap(self,0,0 + i,1) + 32)  #sysytemデータで使用しているフォントはアスキーコードと実際の文字列に32の差が出るので注意(フォントマップの横幅が32キャラだから)
            namestr = namestr + st
        ...
        for i in range(len(self.achievement_list)):
            self.achievement_list[i][LIST_ACHIEVE_FLAG] = update_bg.get_chrcode_tilemap(self,0,0,41 + i) - 16
    ...
    def save_data(self):
        ...
        update_bg.set_chrcode_tilemap(self,0, 0,120,self.game_difficulty + 16)                 #難易度書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,121,self.stage_number + 16)                    #スタートステージ数書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,122,self.stage_loop + 16)                      #スタート周回数書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,126,self.debug_menu_status + 16)               #デバッグメニュー表示フラグ書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,127,self.boss_collision_rect_display_flag + 16)#ボス当たり判定矩形表示フラグ書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,128,self.bg_collision_Judgment_flag + 16)      #BGとの当たり判定フラグ書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,129,self.boss_test_mode + 16)                  #ボステストモードフラグ書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,130,self.no_enemy_mode + 16)                   #敵が出ないモードフラグ書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,131,self.god_mode_status + 16)                 #ゴッドモードフラグ書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,132,self.fullscreen_mode + 16)                 #フルスクリーン起動フラグ書き込み
        update_bg.set_chrcode_tilemap(self,0, 0,133,self.ctrl_type + 16)                       #パッドコントロールタイプ書き込み
        ...
        update_bg.set_chrcode_tilemap(self,0 ,0,136,self.language + 16)                        #選択言語書き込み
        ...
        #メダル所有フラグの書き込み
        for i in range(10):
            update_bg.set_chrcode_tilemap(self,0, 0,210 + i,self.medal_list[i] + 16)
        
        #各機体のスロット装備済みメダルデータの書き込み
        for i in range(LOOK_AT_LOGO): #iは0=J_pythonからLOOK_AT_LOGOまで変化
            for j in range(6):        #jはスロット0からスロット6まで変化
                update_bg.set_chrcode_tilemap(self,0, 23 + j,11 + i,self.ship_equip_slot_list[i][j] + 16)
        ...
        for i in range(GAME_INSAME+1): #iは0からGAME_INSAME+1(6)まで変化する(難易度)
            ...
            for j in range(11):      #jは0から11まで変化する(順位)
                ...
                for k in range(8):   #kは0から8まで変化する(8文字だよ)
                    update_bg.set_chrcode_tilemap(self,tm, 3 + k,5 + j + (i * 16),ord(namestr[k]) - 32)  #sysytemデータで使用しているフォントはアスキーコードと実際の文字列に32の差が出るので注意(フォントマップの横幅が32キャラだから)数字は16の差が出る
                ...
                for l in range(6): #機体に装着されたメダル装備IDの書き込み
                    update_bg.set_chrcode_tilemap(self,tm, 41 + l,5 + j + (i * 16),self.score_board[i][j][LIST_SCORE_BOARD_SHIP_SLOT0 + l] + 16)
        
        #名前の書き込み
        for i in range(8):   #kは0から8まで変化する(8文字だよ)
            update_bg.set_chrcode_tilemap(self,0, 0 + i,1,ord(self.my_name[i]) - 32)  #sysytemデータで使用しているフォントはアスキーコードと実際の文字列に32の差が出るので注意(フォントマップの横幅が32キャラだから)数字は16の差が出る
        ...
        #実績(アチーブメント)の書き込み
        for i in range(len(self.achievement_list)):
            update_bg.set_chrcode_tilemap(self,0, 0,41 + i,self.achievement_list[i][LIST_ACHIEVE_FLAG] + 16)
        ...
        update_bg.set_chrcode_tilemap(self,0, 9,5,sec_1 + 16) #秒の  1の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 8,5,sec_10 + 16) #秒の  10の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 6,5,min_1 + 16) #分の  1の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 5,5,min_10 + 16) #分の  10の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 3,5,hour_1  + 16) #時の   1の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 2,5,hour_10 + 16) #時の   10の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 1,5,hour_100 + 16) #時の   100の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 0,5,hour_1000 + 16) #時の   1000の位を書き込む
        ...
        #総開発テストプレイ時間を書き込んでいく
        update_bg.set_chrcode_tilemap(self,0, 7,3,min_1 + 16) #分の  1の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 6,3,min_10 + 16) #分の  10の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 4,3,hour_1  + 16) #時の   1の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 3,3,hour_10 + 16) #時の   10の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 2,3,hour_100 + 16) #時の   100の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 1,3,hour_1000 + 16) #時の   1000の位を書き込む
        update_bg.set_chrcode_tilemap(self,0, 0,3,hour_10000 + 16) #時の   10000の位を書き込む
