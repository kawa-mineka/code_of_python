...
from update.system             import * #システムデータをセーブするときに使用します
from update.window             import * #各種ウィンドウ作成時に使用するのでインポート
from update.visualscene        import * #ビジュアルシーン作成時に使用するのでインポートします
from update.medal              import * #メダル装備更新で使用するのでインポート
# from update.update_sound       import * #SEのVOLリスト原本を取得しバックアップするために必要なのでインポート
from update.btn_assign         import * #パッドボタン割り当てのリスト更新で使用するのでインポートします
...
class title:
    ...
    def title_init(self):
        ...
        sound.backup_se_vol_list(self)            #各効果音のVOLのリストを原本として保存しておくメソッドの呼び出し
        sound.create_adjustable_se_vol_list(self) #マスターSEボリュームリストを参考にボリューム調整を施したリストを作り上げるメソッドの呼び出し
        ...
        medal.zero_clear_out_of_range_slot(self) #総スロット以上の場所のスロットを空にする関数の呼び出し
        
        if self.title_startup_count == 0:
            window.create(self,Window_id.TITLE_TEXT,0,0)  #タイトルで表示されるテキスト群ウィンドウ(フレームレス)(外枠は表示しない)を作製
            ...
            self.active_window_id = Window_id.TITLE_TEXT  #このウィンドウIDを最前列アクティブなものとする
            self.push_any_btn_flag    = FLAG_OFF          #決定ボタンを押した,離したフラグをオフにする
            self.release_the_btn_flag = FLAG_OFF
            visualscene.create(self,VS_ID_OPENING_STORY1)             #オープニングのストーリーテキストその1のビジュアルシーンを作製する
            visualscene.init_storyboard(self)                         #ストーリーボードの初期化
    ...
    def title(self):
        ...
        if self.title_oscillation_count == 0 and self.title_slash_in_count == 0 and self.display_title_time == 0:
            if self.title_startup_count == 0:
                ...
                window.create(self,Window_id.MAIN_MENU,0,0)         #メニューウィンドウを作製
                ...
                self.active_window_id = Window_id.MAIN_MENU         #このウィンドウIDを最前列アクティブなものとする
                self.push_any_btn_flag    = FLAG_OFF                #決定ボタンを押した,離したフラグをオフにする
                self.release_the_btn_flag = FLAG_OFF
    ...
    def title_hit_any_btn(self):
        ...
        if self.push_any_btn_flag == FLAG_ON and self.release_the_btn_flag == FLAG_ON:
            pyxel.play(CH0,CURSOR_OK_SE_NORMAL)                   #カーソルOK音を鳴らす
            i = func.search_window_id(self,Window_id.TITLE_TEXT)
            self.window[i].vy = 0.3                             #Window_id.TITLE_TEXTウィンドウを下にフッ飛ばしていく
    ...
    def title_menu_select(self):
        ...
        if   self.cursor_menu_layer == MENU_LAYER0: #メニューが0階層目の選択分岐
            if   self.cursor_decision_item_y == MENU_GAME_START:        #GAME STARTが押されたら
                ...
                if func.search_window_id(self,Window_id.SELECT_STAGE_MENU) == -1: #SELECT_STAGE_MENUウィンドウが存在しないのなら・・
                    window.move_left_main_menu_window(self) #メインメニューウィンドウを左にずらす関数の呼び出し
                    
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「SELECT STAGE」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)       #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.SELECT_STAGE_MENU,90,60)  #ステージセレクトウィンドウの作製
                    ...
                    self.active_window_id = Window_id.SELECT_STAGE_MENU #このウィンドウIDを最前列アクティブなものとする
                ...
                if func.search_window_id(self,Window_id.SELECT_LOOP_MENU) == -1: #SELECT_LOOP_MENUウィンドウが存在しないのなら・・
                    window.move_left_main_menu_window(self) #メインメニューウィンドウを左にずらす関数の呼び出し
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「SELECT LOOP」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.SELECT_LOOP_MENU,112,66)      #ループセレクトウィンドウの作成
                    #選択カーソル表示をon,カーソルは上下移動のみ,,カーソル移動ステップはx4,y7,いま指示しているアイテムナンバーは0の「1」
                    ...
                    self.active_window_id = Window_id.SELECT_LOOP_MENU  #このウィンドウIDを最前列でアクティブなものとする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                
            elif self.cursor_decision_item_y == MENU_BOSS_MODE:         #BOSS MODEが押されて
                if func.search_window_id(self,Window_id.BOSS_MODE_MENU) == -1: #BOSS MODEウィンドウが存在しないのなら・・
                    window.move_left_main_menu_window(self) #メインメニューウィンドウを左にずらす関数の呼び出し
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「BOSS MODE」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.BOSS_MODE_MENU,99,59)        #ボスモードon/offウィンドウの作製
                    ...
                    self.active_window_id = Window_id.BOSS_MODE_MENU    #このウィンドウIDを最前列でアクティブなものとする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                
            elif self.cursor_decision_item_y == MENU_HITBOX:            #HITBOXが押されて....
                if func.search_window_id(self,Window_id.HITBOX_MENU) == -1: #HITBOXウィンドウが存在しないのなら・・
                    window.move_left_main_menu_window(self) #メインメニューウィンドウを左にずらす関数の呼び出し
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「HITBOX」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.HITBOX_MENU,99,59)           #ヒットボックスon/offウィンドウの作製
                    ...
                    self.active_window_id = Window_id.HITBOX_MENU       #このウィンドウIDを最前列でアクティブなものとする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                
            elif self.cursor_decision_item_y == MENU_DIFFICULTY:        #DIFFICULTYが押されて
                if func.search_window_id(self,Window_id.SELECT_DIFFICULTY) == -1: #SELECT_DIFFICULTYウィンドウが存在しないのなら・・
                    window.move_left_main_menu_window(self) #メインメニューウィンドウを左にずらす関数の呼び出し
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「DIFFICULTY」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.SELECT_DIFFICULTY,93,52)     #「SELECT DIFFICULTY」ウィンドウの作製
                    ...
                    self.active_window_id = Window_id.SELECT_DIFFICULTY #このウィンドウIDを最前列でアクティブなものとする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                
            elif self.cursor_decision_item_y == MENU_SELECT_SHIP:       #SELECT SHIPが押されて・・・
                if func.search_window_id(self,Window_id.SELECT_SHIP) == -1: #SELECT_SHIPウィンドウが存在しないのなら・・
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「SELECT SHIP」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.SELECT_SHIP,0,0)     #「SELECT SHIP」ウィンドウの作製
                    ...
                    self.active_window_id = Window_id.SELECT_SHIP #このウィンドウIDを最前列でアクティブなものとする
                    #!check##############################################################################################
                    window.change_window_priority_top(self,Window_id.SELECT_SHIP) #ウィンドウ「SELECT SHIP」のプライオリティを最前面にする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                
            elif self.cursor_decision_item_y == MENU_SCORE_BOARD:       #SCORE BOARDが押されて...
                if func.search_window_id(self,Window_id.SCORE_BOARD) == -1: #SCORE_BOARDウィンドウが存在しないのなら・・
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「SCORE BOARD」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create_score_board(self,GAME_NORMAL)                #スコアボードウィンドウを育成=============
                    ...
                    self.active_window_id = Window_id.SCORE_BOARD       #このウィンドウIDを最前列でアクティブなものとする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                
            elif self.cursor_decision_item_y == MENU_NAME_ENTRY:        #NAME ENTRYが押されて...
                if func.search_window_id(self,Window_id.INPUT_YOUR_NAME) == -1: #INPUT_YOUR_NAMEウィンドウが存在しないのなら・・
                    window.move_left_main_menu_window(self) #メインメニューウィンドウを左にずらす関数の呼び出し
                    ...
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.INPUT_YOUR_NAME,80,52)       #「ENTER YOUR NAME」ウィンドウの作製
                    ...
                    self.active_window_id = Window_id.INPUT_YOUR_NAME   #このウィンドウIDを最前列でアクティブなものとする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                
            elif self.cursor_decision_item_y == MENU_CONFIG:            #CONFIGが押されて
                if func.search_window_id(self,Window_id.CONFIG) == -1: #SELECT_CONFIGウィンドウが存在しないのなら・・
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「CONFIG」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.CONFIG,4,4)                #「CONFIG」ウィンドウの作製
                    ...
                    self.active_window_id = Window_id.CONFIG #このウィンドウIDを最前列でアクティブなものとする
                    window.change_window_priority_top(self,Window_id.CONFIG) #ウィンドウ「CONFIG」のプライオリティを最前面にする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                ...
                if func.search_window_id(self,Window_id.MEDAL_LIST) == -1 and func.search_window_id(self,Window_id.EQUIPMENT) == -1: #MEDAL_LISTウィンドウとEQUIPMENTウィンドウが同時に存在しないのなら・・
                    window.move_left_main_menu_window(self) #メインメニューウィンドウを左にずらす関数の呼び出し
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「MEDAL_LIST」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)   #メインメニューのカーソルデータをPUSH
                    
                    window.create(self,Window_id.MEDAL_LIST,17,48) #「MEDAL_LIST」ウィンドウの作製
                    window.create(self,Window_id.EQUIPMENT,17,17)  #「EQUIPMENT」 ウィンドウの作製
                    ...
                    self.active_window_id = Window_id.MEDAL_LIST #MEDAL_LISTウィンドウを最前列でアクティブなものとする
                ...
                if func.search_window_id(self,Window_id.STATUS) == -1: #STATUSウィンドウが存在しないのなら・・
                    self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「STATUS」を前のレイヤー選択アイテムとしてコピーする
                    func.push_cursor_data(self,Window_id.MAIN_MENU)         #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.STATUS,4,0)           #STATUSウィンドウの作製
                    ...
                    self.active_window_id = Window_id.STATUS       #このウィンドウIDを最前列でアクティブなものとする
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                ...
                if func.search_window_id(self,Window_id.EXIT) == -1: #ゲーム終了(退出)ウィンドウが存在しないのなら・・
                    ...
                    func.push_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPUSH
                    window.create(self,Window_id.EXIT,50,59)                  #ゲーム終了(退出)ウィンドウの作製
            ...
            if  self.cursor_pre_decision_item_y  == MENU_SELECT_STAGE:
                if   self.cursor_decision_item_y == MENU_SELECT_STAGE_1:
                    ...
                    window.move_right_main_menu_window(self) #メインメニューウィンドウを右にずらす関数の呼び出し
                    
                    i = func.search_window_id(self,Window_id.SELECT_STAGE_MENU)
                    self.window[i].vx = 0.6            #Window_id.SELECT_STAGE_MENUウィンドウを右にフッ飛ばしていく
                    ...
                    func.pop_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPOP
                    self.cursor_pre_decision_item_y = UNSELECTED
                    self.active_window_id = Window_id.MAIN_MENU #1階層前メインメニューウィンドウIDを最前列でアクティブなものとする
                    ...
                    i = func.search_window_id(self,Window_id.SELECT_STAGE_MENU)
                    self.window[i].vx = 0.3            #Window_id.SELECT_STAGE_MENUウィンドウを右にフッ飛ばしていく
                    self.window[i].vx_accel = 1.1
                    self.window[i].vy = 0.1 * self.stage_number
                    self.window[i].vy_accel = 1.1
                    self.window[i].window_status = WINDOW_CLOSE
                ...
                if   self.cursor_decision_item_y == MENU_SELECT_LOOP_1:
                    ...
                    window.move_right_main_menu_window(self) #メインメニューウィンドウを右にずらす関数の呼び出し
                    i = func.search_window_id(self,Window_id.SELECT_LOOP_MENU)
                    self.window[i].vx = 0.3            #Window_id.SELECT_LOOP_MENUウィンドウを右にフッ飛ばしていく
                ...
                if   self.cursor_decision_item_y == MENU_BOSS_MODE_OFF:
                    ...
                    i = func.search_window_id(self,Window_id.BOSS_MODE_MENU)
                    self.window[i].vx = 0.3            #Window_id.BOSS_MODE_MENUウィンドウを右にフッ飛ばしていく
                    self.window[i].vx_accel = 1.2
                    self.window[i].window_status = WINDOW_CLOSE
                    self.window[i].comment_flag = COMMENT_FLAG_OFF
                    ...
                    func.create_master_flag_list(self) #フラグ＆データ関連のマスターリスト作成関数を呼び出す
                    i = func.search_window_id(self,Window_id.BOSS_MODE_MENU)
                    self.window[i].vx = 0.3            #Window_id.BOSS_MODE_MENUウィンドウを右下にフッ飛ばしていく
                    self.window[i].vx_accel = 1.2
                ...
                if   self.cursor_decision_item_y == MENU_HITBOX_OFF:
                    ...
                    window.move_right_main_menu_window(self) #メインメニューウィンドウを右にずらす関数の呼び出し
                    func.create_master_flag_list(self) #フラグ＆データ関連のマスターリスト作成関数を呼び出す
                    i = func.search_window_id(self,Window_id.HITBOX_MENU)
                    self.window[i].vx = 0.3            #Window_id.HITBOX_MENUウィンドウを右にフッ飛ばしていく
                    self.window[i].vx_accel = 1.2
                    self.window[i].window_status = WINDOW_CLOSE
                    self.window[i].comment_flag = COMMENT_FLAG_OFF
                    self.window[i].flag_list = self.master_flag_list #ボステストフラグを更新→マスターフラグデータリスト更新→ウィンドウのフラグリストに書き込んで更新します
                    ...
                    func.create_master_flag_list(self) #フラグ＆データ関連のマスターリスト作成関数を呼び出す
                    i = func.search_window_id(self,Window_id.HITBOX_MENU)
                    self.window[i].vx = 0.3            #Window_id.HITBOX_MENUウィンドウを右下にフッ飛ばしていく
                    self.window[i].vx_accel = 1.2
                    self.window[i].vy = 0.2
                    self.window[i].vy_accel = 1.2
                    self.window[i].window_status = WINDOW_CLOSE
                ...
                if   self.cursor_decision_item_y == GAME_VERY_EASY:
                    ...
                    window.move_right_main_menu_window(self) #メインメニューウィンドウを右にずらす関数の呼び出し
                    func.create_master_flag_list(self) #フラグ＆データ関連のマスターリスト作成関数を呼び出す
                    i = func.search_window_id(self,Window_id.SELECT_DIFFICULTY)
                    self.window[i].vx = 0.3            #Window_id.SELECT_DIFFICULTYウィンドウを右にフッ飛ばしていく
                    self.window[i].vx_accel = 1.2
                ...
                if   self.cursor_page == GAME_VERY_EASY and self.cursor_pre_page != self.cursor_page: #前に表示していたページ数と現在のページ数に変化があった時だけ
                    if self.cursor_move_data == PAD_RIGHT:
                        func.all_move_window(self,Window_id.SCORE_BOARD, 0.3,0, 1.2,0) #すべてのSCORE_BOARDウィンドウを右方向にフッ飛ばしていく
                    else:
                        func.all_move_window(self,Window_id.SCORE_BOARD,-0.3,0, 1.2,0) #すべてのSCORE_BOARDウィンドウを左方向にフッ飛ばしていく
                    
                    window.create_score_board(self,GAME_VERY_EASY)                           #スコアボードウィンドウ育成
                    ...
                    window.create_score_board(self,GAME_EASY)                           #スコアボードウィンドウ育成
                    self.cursor_pre_page = self.cursor_page              #前回のページ数を保存
                    ...
                    window.create_score_board(self,GAME_NORMAL)                           #スコアボードウィンドウ育成
                    self.cursor_pre_page = self.cursor_page              #前回のページ数を保存
                    ...
                    window.create_score_board(self,GAME_HARD)                           #スコアボードウィンドウ育成
                    self.cursor_pre_page = self.cursor_page              #前回のページ数を保存
                    ...
                    window.create_score_board(self,GAME_VERY_HARD)                           #スコアボードウィンドウ育成
                    self.cursor_pre_page = self.cursor_page              #前回のページ数を保存
                    ...
                    window.create_score_board(self,GAME_INSAME)                           #スコアボードウィンドウ育成
                    self.cursor_pre_page = self.cursor_page              #前回のページ数を保存
                    ...
                    func.all_move_window(self,Window_id.SCORE_BOARD,0,0.3,0,1.2) #すべてのSCORE_BOARDウィンドウを下方向にフッ飛ばしていく
                    func.pop_cursor_data(self,Window_id.MAIN_MENU)               #メインメニューのカーソルデータをPOP
                ...
                func.all_move_window(self,Window_id.INPUT_YOUR_NAME,0.2,0.3,1.2,1.2) #すべてのINPUT_YOUR_NAMEウィンドウを右下方向にフッ飛ばしていく
                func.pop_cursor_data(self,Window_id.MAIN_MENU)          #メインメニューのカーソルデータをPOP
                self.cursor_pre_decision_item_y = UNSELECTED
                pyxel.play(CH0,self.window[self.active_window_index].cursor_ok_se)#カーソルOK音を鳴らす
                self.active_window_id = Window_id.MAIN_MENU #1階層前メインメニューウィンドウIDを最前列でアクティブなものとする
                ...
                if   self.cursor_decision_item_y == MENU_CONFIG_SCREEN_MODE:
                    ...
                    window.change_window_priority_normal(self,Window_id.CONFIG) #CONFIGウィンドウの表示優先度を「normal」にする
                    if func.search_window_id(self,Window_id.JOYPAD_ASSIGN) == -1: #「JOYPAD_ASSIGN」ウィンドウが存在しないのなら・・
                        ...
                        func.push_cursor_data(self,Window_id.CONFIG)             #「CONFIG」ウィンドウのカーソルデータをPUSH
                        window.create(self,Window_id.JOYPAD_ASSIGN,0,0) #「JOYPAD ASSIGN」ウィンドウの作製
                        #選択カーソル表示をon,カーソルは上下移動のみ,,カーソル移動ステップはx4,y7,いま指示しているアイテムナンバーは2の「NORMAL」
                        ...
                        self.active_window_id = Window_id.JOYPAD_ASSIGN #このウィンドウIDを最前列でアクティブなものとする
                        pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                    ...
                    window.change_window_priority_normal(self,Window_id.CONFIG) #CONFIGウィンドウの表示優先度を「normal」にする
                    if func.search_window_id(self,Window_id.INITIALIZE) == -1: #「INITIALIZE」ウィンドウが存在しないのなら・・
                        self.cursor_pre_pre_decision_item_y = self.cursor_pre_decision_item_y #前のレイヤー選択アイテム「CONFIG」を前の前のレイヤー選択アイテムとして保存する
                        ...
                        window.create(self,Window_id.INITIALIZE,ox,oy)    #「INITIALIZE」ウィンドウの作製
                        #選択カーソル表示をon,カーソルは上下移動のみ,,カーソル移動ステップはx4,y7,いま指示しているアイテムナンバーは2の「NORMAL」
                        ...
                        self.active_window_id = Window_id.INITIALIZE #このウィンドウIDを最前列でアクティブなものとする
                        pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                    ...
                    window.change_window_priority_normal(self,Window_id.CONFIG) #CONFIGウィンドウの表示優先度を「normal」にする
                    i = func.search_window_id(self,Window_id.CONFIG)
                    self.window[i].vx = -0.5            #Window_id.CONFIGウィンドウを左下にフッ飛ばしていく
                    self.window[i].vx_accel = 1.1
                    self.window[i].vy = 0.1
                    self.window[i].vy_accel = 1.1
                    self.window[i].window_status = WINDOW_CLOSE
                ...
                if self.cursor_decision_item_y == 4 and 6 <= self.cursor_decision_item_x <= 8: #OKボタンが押されたときの処理
                    window.move_right_main_menu_window(self) #メインメニューウィンドウを右にずらす関数の呼び出し
                    func.create_master_flag_list(self) #フラグ＆データ関連のマスターリスト作成関数を呼び出す
                    i = func.search_window_id(self,Window_id.MEDAL_LIST)
                    ...
                    i = func.search_window_id(self,Window_id.EQUIPMENT)
                    ...
                    i = func.search_window_id(self,Window_id.MEDAL_LIST) #メダルリストウィンドウのインデックス値取得
                    if self.window[i].comment_disp_flag[self.cursor_decision_item_y][self.cursor_decision_item_x] == DISP_ON: #決定ボタンが押された位置のコメント表示フラグが立っている時はそこの部分のアイテムは所持しているので....
                        ...
                        if num != MEDAL_NO_SLOT:               #MEDAL_NO_SLOT(メダルは何もない)以外の場合は
                            medal.equip_medal_playing_ship(self,num) #アイテムID(この場合はメダルID)を書き込んで更新する
                    
                    i = func.search_window_id(self,Window_id.EQUIPMENT)    #メダル装備ウィンドウのインデックス値取得
                    self.window[i].ship_list = self.playing_ship_list      #ウィンドウクラスのシップリストも更新してやります
                    medal.medal_effect_plus_medallion(self)                #装備メダルに「スロット数を拡張するメダル」があればスロット数を増やし,無ければスロット数を初期状態にする関数の呼び出し
                    medal.zero_clear_out_of_range_slot(self)               #現在の総メダルスロット以上のスロット部分をゼロクリアしてメダルなし状態にする関数の呼び出し
                    ...
                    i = func.search_window_id(self,Window_id.EQUIPMENT)             #メダル装備ウィンドウのインデックス値取得
                    medal.purge_medal_playing_ship(self,self.cursor_decision_item_x) #決定ボタンが押された位置のスロットを空にする関数の呼び出し
                    medal.zero_clear_out_of_range_slot(self)               #現在の総メダルスロット以上のスロット部分をゼロクリアしてメダルなし状態にする関数の呼び出し
                    medal.playing_ship_medal_left_justified(self)          #装備スロットに空が出来たので左に詰めていく関数を呼び出す              
                ...
                i = func.search_window_id(self,Window_id.STATUS)
                ...
                window.change_window_priority_normal(self,Window_id.SELECT_SHIP) #SELECT_SHIPウィンドウの表示優先度を「normal」にする
                i = func.search_window_id(self,Window_id.SELECT_SHIP)
                ...
                if   self.cursor_decision_item_y == MENU_EXIT_NO:
                    ...
                    self.window[i].vy = -0.3            #Window_id.EXITウィンドウを右上にフッ飛ばしていく
                    self.window[i].vy_accel = 1.2
                    ...
                    i = func.search_window_id(self,Window_id.EXIT)
                    self.window[i].vy = -0.1            #Window_id.EXITウィンドウを右上にフッ飛ばしていく
            ...
            if   self.cursor_pre_pre_decision_item_y == MENU_CONFIG:
                if   self.cursor_pre_decision_item_y == MENU_CONFIG_INITIALIZE and self.cursor_decision_item_y == MENU_CONFIG_INITIALIZE_RETURN:
                    window.change_window_priority_normal(self,Window_id.INITIALIZE) #INITIALIZEウィンドウの表示優先度を「normal」にする
                    func.create_master_flag_list(self)                    #フラグ＆データ関連のマスターリスト作成関数を呼び出す
                    i = func.search_window_id(self,Window_id.INITIALIZE)
                    self.window[i].vx = 0.3                           #Window_id.INITIALIZEウィンドウを右上にフッ飛ばしていく
                    self.window[i].vx_accel = 1.2
                    self.window[i].vy = -0.2
                    self.window[i].vy_accel = 1.2
                    self.window[i].window_status = WINDOW_CLOSE
                    ...
                    func.pop_cursor_data(self,Window_id.CONFIG)                          #「CONFIG」ウィンドウのカーソルデータをPOP
                    ...
                    pyxel.play(CH0,self.window[self.active_window_index].cursor_ok_se)#カーソルOK音を鳴らす
                    self.active_window_id = Window_id.CONFIG                        #1階層前の「CONFIG」ウィンドウIDを最前列でアクティブなものとする
                    window.change_window_priority_top(self,Window_id.CONFIG) #CONFIGウィンドウの表示優先度を「TOP」にする
                elif self.cursor_pre_decision_item_y == MENU_CONFIG_INITIALIZE and self.cursor_decision_item_y == MENU_CONFIG_INITIALIZE_SCORE:
                    if func.search_window_id(self,Window_id.PRINT_INIT_SCORE) == -1 and func.search_window_id(self,Window_id.SELECT_YES_NO) == -1: #PRINT_INIT_SCOREウィンドウとSELECT_YES_NOウィンドウが同時に存在しないのなら・・
                        ...
                        func.push_cursor_data(self,Window_id.MAIN_MENU)           #メインメニューのカーソルデータをPUSH
                        window.create(self,Window_id.SELECT_YES_NO,34,25)    #「SELECT_YES_NO」 ウィンドウの作製
                        window.create(self,Window_id.PRINT_INIT_SCORE,20,10) #「PRINT_INIT_SCORE」ウィンドウの作製
                        ...
                        self.active_window_id = Window_id.SELECT_YES_NO #SELECT_YES_NOウィンドウを最前列でアクティブなものとする
                        pyxel.play(CH0,self.window[self.active_window_index].cursor_push_se)#カーソルボタンプッシュ音を鳴らす
                elif self.cursor_pre_decision_item_y == MENU_CONFIG_INITIALIZE and self.cursor_decision_item_y == MENU_CONFIG_INITIALIZE_NAME:
                    if func.search_window_id(self,Window_id.PRINT_INIT_NAME) == -1 and func.search_window_id(self,Window_id.SELECT_YES_NO) == -1: #PRINT_INIT_NAMEウィンドウとSELECT_YES_NOウィンドウが同時に存在しないのなら・・
                        self.cursor_pre_pre_pre_decision_item_y = self.cursor_pre_pre_decision_item_y #2回前に選択されたアイテムを3回前に選択されたアイテムにコピー 
                        self.cursor_pre_pre_decision_item_y = self.cursor_pre_decision_item_y #1回前に選択されたアイテムを2回前に選択されたアイテムにコピー
                        self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「INITIALIZE_NAME」を前のレイヤー選択アイテムとしてコピーする
                        func.push_cursor_data(self,Window_id.MAIN_MENU)           #メインメニューのカーソルデータをPUSH
                        window.create(self,Window_id.SELECT_YES_NO,34,25)    #「SELECT_YES_NO」 ウィンドウの作製
                        window.create(self,Window_id.PRINT_INIT_NAME,20,10) #「PRINT_INIT_NAME」ウィンドウの作製
                        #カーソルは点滅囲み矩形タイプ,カーソルは4方向,カーソル移動ステップはx4y7,いま指し示しているitem_x,item_yは(0,0)
                        #まだボタンも押されておらず未決定状態なのでdecision_item_yはUNSELECTED,x最大項目数は1項目なので1-1=0を代入,y最大項目数は2項目なので2-1=1を代入,メニューの階層が増えたのでMENU_LAYER2からMENU_LAYER3にします
                        func.set_cursor_data(self,CURSOR_TYPE_NORMAL,CURSOR_MOVE_UD,38,35,STEP4,STEP7,0,0,0,0,UNSELECTED,UNSELECTED,0,2-1,0,MENU_LAYER3)
                    ...
                    if func.search_window_id(self,Window_id.PRINT_INIT_ALL) == -1 and func.search_window_id(self,Window_id.SELECT_YES_NO) == -1: #PRINT_INIT_ALLウィンドウとSELECT_YES_NOウィンドウが同時に存在しないのなら・・
                        self.cursor_pre_pre_pre_decision_item_y = self.cursor_pre_pre_decision_item_y #2回前に選択されたアイテムを3回前に選択されたアイテムにコピー 
                        self.cursor_pre_pre_decision_item_y = self.cursor_pre_decision_item_y #1回前に選択されたアイテムを2回前に選択されたアイテムにコピー
                        self.cursor_pre_decision_item_y = self.cursor_decision_item_y #現時点で選択されたアイテム「INITIALIZE_NAME」を前のレイヤー選択アイテムとしてコピーする
                        func.push_cursor_data(self,Window_id.MAIN_MENU)           #メインメニューのカーソルデータをPUSH
                        window.create(self,Window_id.SELECT_YES_NO,34,25)    #「SELECT_YES_NO」 ウィンドウの作製
                        window.create(self,Window_id.PRINT_INIT_ALL,20,10) #「PRINT_INIT_ALL」ウィンドウの作製
                        #カーソルは点滅囲み矩形タイプ,カーソルは4方向,カーソル移動ステップはx4y7,いま指し示しているitem_x,item_yは(0,0)
                        #まだボタンも押されておらず未決定状態なのでdecision_item_yはUNSELECTED,x最大項目数は1項目なので1-1=0を代入,y最大項目数は2項目なので2-1=1を代入,メニューの階層が増えたのでMENU_LAYER2からMENU_LAYER3にします
                        func.set_cursor_data(self,CURSOR_TYPE_NORMAL,CURSOR_MOVE_UD,38,35,STEP4,STEP7,0,0,0,0,UNSELECTED,UNSELECTED,0,2-1,0,MENU_LAYER3)
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定の場合は何もしない
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_SHOT_AND_SUB_WEAPON)
                        
                        pyxel.play(CH0,self.window[self.active_window_index].cursor_ok_se)  #カーソルOK音を鳴らす
                        i = func.search_window_id(self,Window_id.JOYPAD_ASSIGN)           #ジョイパッドボタン割り当て設定ウィンドウのインデックス値を取得
                        self.window[i].pad_assign_list = self.pad_assign_list             #ウィンドウクラスのパッドアサインリストも更新してやります
                        window.refresh_pad_assign_graph_list(self)                 #パッドアサイングラフイックリストを更新して新しいものにします(self.pad_assign_listに依存しているため)
                        i = func.search_window_id(self,Window_id.JOYPAD_ASSIGN)           #ジョイパッドボタン割り当て設定ウィンドウのインデックス値を取得
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定は除く
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_MISSILE)
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定は除く
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_MAIN_WEAPON_CHANGE)
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定は除く
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_SUB_WEAPON_CHANGE)
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定は除く
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_SPEED_CHANGE)
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定は除く
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_PAUSE)
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定は除く
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_CHANGE_CLAW_STYLE)
                    ...
                    if self.cursor_button_data != BTN_KEYBOARD_SPACE: #スペースキーで決定は除く
                        ...
                        btn_assign.list(self,self.cursor_button_data,ACT_CHANGE_CLAW_INTERVAL)
                    ...
                    window.change_window_priority_normal(self,MENU_CONFIG_JOYPAD_ASSIGN) #MENU_CONFIG_JOYPAD_ASSIGNウィンドウの表示優先度を「normal」にする
                    func.create_master_flag_list(self)                              #フラグ＆データ関連のマスターリスト作成関数を呼び出す
            ...
            if   self.cursor_pre_pre_pre_decision_item_y == MENU_CONFIG and self.cursor_pre_pre_decision_item_y == MENU_CONFIG_INITIALIZE and self.cursor_pre_decision_item_y == MENU_CONFIG_INITIALIZE_SCORE and self.cursor_decision_item_y == 0:
                ...
                i = func.search_window_id(self,Window_id.PRINT_INIT_SCORE)
                ...
                self.window[i].window_status = WINDOW_CLOSE
                self.window[i].comment_flag = COMMENT_FLAG_OFF
                i = func.search_window_id(self,Window_id.SELECT_YES_NO)
                ...
                self.window[i].window_status = WINDOW_CLOSE
                self.window[i].comment_flag = COMMENT_FLAG_OFF
                func.pop_cursor_data(self,Window_id.INITIALIZE)   #1階層前のカーソルデータをPOP
                ...
                self.active_window_id = Window_id.INITIALIZE #1階層前のイニシャライズウィンドウIDを最前列でアクティブなものとする
                pyxel.play(CH0,self.window[self.active_window_index].cursor_cancel_se)#カーソルキャンセル音を鳴らす
                ...
                i = func.search_window_id(self,Window_id.PRINT_INIT_NAME)
                ...
                i = func.search_window_id(self,Window_id.PRINT_INIT_ALL)
