...
class update_replay:
    ...
    def data_file_load(self):
        ...
        for i in range(self.replay_stage_num + 1):
            self.replay_mode_stage_data[i][ST_SCORE]           = func.read_system_data_num(self,5   -1+10,10+i,0, 10)        #座標(5,10+i)から10ケタのスコア(整数)を読み込みます
            self.replay_mode_stage_data[i][ST_MY_SHIELD]       = func.read_system_data_num(self,16  -1 +5,10+i,0,  5)        #座標(16,10+i)から5ケタのシールド値を読み込みます
            self.replay_mode_stage_data[i][ST_MY_SPEED]        = func.read_system_data_num(self,22  -1 +3,10+i,0,  3) // 100 #座標(22,10+i)から3ケタの自機スピード(0.75とか1.25とか小数点第2位まで行くので100でわった値を読み込みます)
            
            self.replay_mode_stage_data[i][ST_SELECT_SHOT_ID]  = func.read_system_data_num(self,27  -1 +2,10+i,0,  2)    #座標(27,10+i)から2ケタのショットIDを読み込みます
            
            self.replay_mode_stage_data[i][ST_SHOT_EXP]                 = func.read_system_data_num(self,31  -1 +4,10+i,0,  4)         #座標(31,10+i)から4ケタのショットの経験値を読み込みます
            self.replay_mode_stage_data[i][ST_SHOT_LEVEL]               = func.read_system_data_num(self,36  -1 +2,10+i,0,  2)         #座標(36,10+i)から2ケタのショットのレベルを読み込みます
            self.replay_mode_stage_data[i][ST_SHOT_SPEED_MAGNIFICATION] = func.read_system_data_num(self,39  -1 +5,10+i,0,  5) // 1000 #座標(39,10+i)から5ケタのショットのスピードに掛ける倍率を読み込みます(小数点第3位まで行くので1000で割った値を読み込みます)
            self.replay_mode_stage_data[i][ST_SHOT_RAPID_OF_FIRE]       = func.read_system_data_num(self,45  -1 +2,10+i,0,  2)         #座標(45,10+i)から2ケタのショットの連射数を読み込みます
            
            self.replay_mode_stage_data[i][ST_MISSILE_EXP]                 = func.read_system_data_num(self,49  -1 +4,10+i,0,  4)         #座標(49,10+i)から4ケタのミサイルの経験値を読み込みます
            self.replay_mode_stage_data[i][ST_MISSILE_LEVEL]               = func.read_system_data_num(self,54  -1 +2,10+i,0,  2)         #座標(54,10+i)から2ケタのミサイルのレベルを読み込みます
            self.replay_mode_stage_data[i][ST_MISSILE_SPEED_MAGNIFICATION] = func.read_system_data_num(self,57  -1 +5,10+i,0,  5) // 1000 #座標(57,10+i)から5ケタのミサイルのスピードに掛ける倍率を読み込みます(小数点第3位まで行くので1000で割った値を読み込みます)
            self.replay_mode_stage_data[i][ST_MISSILE_RAPID_OF_FIRE]       = func.read_system_data_num(self,63  -1 +2,10+i,0,  2)         #座標(63,10+i)から2ケタのミサイルの連射数を読み込みます
            
            self.replay_mode_stage_data[i][ST_SELECT_SUB_WEAPON_ID] = func.read_system_data_num(self,68  -1 +2,10+i,0,  2)   #座標(68,10+i)から2ケタの現在使用しているサブウェポンのIDナンバーを読み込みます
            
            self.replay_mode_stage_data[i][ST_CLAW_NUMBER]          = func.read_system_data_num(self,74  -1 +2,10+i,0,  2)   #座標(74,10+i)から2ケタのクローの装備数を読み込みます
            self.replay_mode_stage_data[i][ST_CLAW_TYPE]            = func.read_system_data_num(self,71  -1 +2,10+i,0,  2)   #座標(71,10+i)から2ケタのクローのタイプを読み込みます
            self.replay_mode_stage_data[i][ST_CLAW_DIFFERENCE]      = func.read_system_data_num(self,77  -1 +4,10+i,0,  4)   #座標(77,10+i)から4ケタのクロ―同士の角度間隔を読み込みます
            
            self.replay_mode_stage_data[i][ST_TRACE_CLAW_INDEX]        = func.read_system_data_num(self,82  -1 +2,10+i,0,  2)          #座標(82,10+i)から2ケタのトレースクロー（オプション）時のトレース用配列のインデックス値を読み込みます
            self.replay_mode_stage_data[i][ST_TRACE_CLAW_DISTANCE]     = func.read_system_data_num(self,85  -1 +5,10+i,0,  5) // 1000  #座標(85,10+i)から5ケタのトレースクロー同士の間隔を読み込みます(小数点第3位まで行くので1000で割った値を読み込みます)
            
            self.replay_mode_stage_data[i][ST_FIX_CLAW_MAGNIFICATION]  =  func.read_system_data_num(self,91  -1 +5,10+i,0,  5) // 1000 #座標(91,10+i)から5ケタのフイックスクロー同士の間隔の倍率を読み込みます(小数点第3位まで行くので1000で割った値を読み込みます)
            
            self.replay_mode_stage_data[i][ST_REVERSE_CLAW_SVX]        = func.read_system_data_num(self,97  -1 +5,10+i,0,  5) // 1000  #座標(97,10+i)から5ケタのリバースクロー用の攻撃方向ベクトル(x軸)を読み込みます(小数点第3位まで行くので1000で割った値を読み込みます)
            self.replay_mode_stage_data[i][ST_REVERSE_CLAW_SVY]        = func.read_system_data_num(self,103 -1 +5,10+i,0,  5) // 1000  #座標(103,10+i)から5ケタのリバースクロー用の攻撃方向ベクトル(y軸)を読み込みます(小数点第3位まで行くので1000で割った値を読み込みます)
            
            self.replay_mode_stage_data[i][ST_CLAW_SHOT_SPEED]         = func.read_system_data_num(self,109 -1 +5,10+i,0,  5) // 1000  #座標(109,10+i)から5ケタのクローショットのスピードを読み込みます(小数点第3位まで行くので1000で割った値を読み込みます)
            self.replay_mode_stage_data[i][ST_LS_SHIELD_HP]            = func.read_system_data_num(self,115 -1 +4,10+i,0,  4)          #座標(115,10+i)から4ケタのL'sシールドの耐久力を読み込みます
            
            self.replay_mode_stage_data[i][ST_SHIP_ID]                 = func.read_system_data_num(self,144 -1 +3,10+i,0,  3)          #座標(144,10+i)から3ケタの使用している機体のIDナンバーを読み込みます
            self.replay_mode_stage_data[i][ST_SHIP_LEVEL]              = func.read_system_data_num(self,148 -1 +3,10+i,0,  3)          #座標(148,10+i)から3ケタの使用している機体のレベルを読み込みます
            self.replay_mode_stage_data[i][ST_SHIP_EXP]                = func.read_system_data_num(self,152 -1 +4,10+i,0,  4)          #座標(152,10+i)から4ケタの使用している機体の経験値を読み込みます
            
            self.replay_mode_stage_data[i][ST_SLOT1]                   = func.read_system_data_num(self,160 -1 +2,10+i,0,  2) #座標(160,10+i)から2ケタの使用している機体のメダルスロット1のメダルIDを読み込みます
            self.replay_mode_stage_data[i][ST_SLOT2]                   = func.read_system_data_num(self,162 -1 +2,10+i,0,  2) #座標(162,10+i)から2ケタの使用している機体のメダルスロット2のメダルIDを読み込みます
            self.replay_mode_stage_data[i][ST_SLOT3]                   = func.read_system_data_num(self,164 -1 +2,10+i,0,  2) #座標(164,10+i)から2ケタの使用している機体のメダルスロット3のメダルIDを読み込みます
            self.replay_mode_stage_data[i][ST_SLOT4]                   = func.read_system_data_num(self,166 -1 +2,10+i,0,  2) #座標(166,10+i)から2ケタの使用している機体のメダルスロット4のメダルIDを読み込みます
            self.replay_mode_stage_data[i][ST_SLOT5]                   = func.read_system_data_num(self,168 -1 +2,10+i,0,  2) #座標(168,10+i)から2ケタの使用している機体のメダルスロット5のメダルIDを読み込みます
            self.replay_mode_stage_data[i][ST_SLOT6]                   = func.read_system_data_num(self,170 -1 +2,10+i,0,  2) #座標(170,10+i)から2ケタの使用している機体のメダルスロット6のメダルIDを読み込みます
            self.replay_mode_stage_data[i][ST_SLOT7]                   = func.read_system_data_num(self,172 -1 +2,10+i,0,  2) #座標(172,10+i)から2ケタの使用している機体のメダルスロット7のメダルIDを読み込みます
            
            self.replay_mode_stage_data[i][ST_SCORE_STAR_MAG]          = func.read_system_data_num(self,176 -1 +3,10+i,0,  3) #座標(176,10+i)から3ケタのスコアスター得点倍率を読み込みます
            
            pad_data_size = func.read_system_data_num(self,128 -1 +8,10+i,0,  8) #座標(128,10+i)からの8ケタのコントロールパッド入力データが記録されたファイルのデータサイズを読み込みます
    ...
    def data_file_save(self):
        ...
        for i in range(self.replay_stage_num + 1):
            func.write_system_data_num(self,5   -1+10,10+i,0, 10,int(self.replay_mode_stage_data[i][ST_SCORE]))           #座標(5,10+i)に10ケタのスコア(整数)を書き込みます
            func.write_system_data_num(self,16  -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_MY_SHIELD]))       #座標(16,10+i)に5ケタのシールド値を書き込みます
            func.write_system_data_num(self,22  -1 +3,10+i,0,  3,int(self.replay_mode_stage_data[i][ST_MY_SPEED]*100))    #座標(22,10+i)に3ケタの自機スピード(0.75とか1.25とか小数点第2位まで行くので100倍した値を書き込みます)
            
            func.write_system_data_num(self,27  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SELECT_SHOT_ID]))  #座標(27,10+i)に2ケタのショットIDを書き込みます
            
            func.write_system_data_num(self,31  -1 +4,10+i,0,  4,int(self.replay_mode_stage_data[i][ST_SHOT_EXP]))              #座標(31,10+i)に4ケタのショットの経験値を書き込みます
            func.write_system_data_num(self,36  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SHOT_LEVEL]))            #座標(36,10+i)に2ケタのショットのレベルを書き込みます
            func.write_system_data_num(self,39  -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_SHOT_SPEED_MAGNIFICATION] * 1000)) #座標(39,10+i)に5ケタのショットのスピードに掛ける倍率を書き込みます(小数点第3位まで行くので1000倍した値を書き込みます)
            func.write_system_data_num(self,45  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SHOT_RAPID_OF_FIRE]))    #座標(45,10+i)に2ケタのショットの連射数を書き込みます
            
            func.write_system_data_num(self,49  -1 +4,10+i,0,  4,int(self.replay_mode_stage_data[i][ST_MISSILE_EXP]))                  #座標(49,10+i)に4ケタのミサイルの経験値を書き込みます
            func.write_system_data_num(self,54  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_MISSILE_LEVEL]))                #座標(54,10+i)に2ケタのミサイルのレベルを書き込みます
            func.write_system_data_num(self,57  -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_MISSILE_SPEED_MAGNIFICATION]* 1000))  #座標(57,10+i)に5ケタのミサイルのスピードに掛ける倍率を書き込みます(小数点第3位まで行くので1000倍した値を書き込みます)
            func.write_system_data_num(self,63  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_MISSILE_RAPID_OF_FIRE]))        #座標(63,10+i)に2ケタのミサイルの連射数を書き込みます
            
            func.write_system_data_num(self,68  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SELECT_SUB_WEAPON_ID]))   #座標(68,10+i)に2ケタの現在使用しているサブウェポンのIDナンバーを書き込みます
            
            func.write_system_data_num(self,74  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_CLAW_NUMBER]))            #座標(74,10+i)に2ケタのクローの装備数を書き込みます
            func.write_system_data_num(self,71  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_CLAW_TYPE]))              #座標(71,10+i)に2ケタのクローのタイプを書き込みます
            func.write_system_data_num(self,77  -1 +4,10+i,0,  4,int(self.replay_mode_stage_data[i][ST_CLAW_DIFFERENCE]))        #座標(77,10+i)に4ケタのクロ―同士の角度間隔を書き込みます
            
            func.write_system_data_num(self,82  -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_TRACE_CLAW_INDEX]))       #座標(82,10+i)に2ケタのトレースクロー（オプション）時のトレース用配列のインデックス値を書き込みます
            func.write_system_data_num(self,85  -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_TRACE_CLAW_DISTANCE] * 1000))    #座標(85,10+i)に5ケタのトレースクロー同士の間隔を書き込みます(小数点第3位まで行くので1000倍した値を書き込みます)
            
            func.write_system_data_num(self,91  -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_FIX_CLAW_MAGNIFICATION] * 1000)) #座標(91,10+i)に5ケタのフイックスクロー同士の間隔の倍率を書き込みます(小数点第3位まで行くので1000倍した値を書き込みます)
            
            func.write_system_data_num(self,97  -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_REVERSE_CLAW_SVX] * 1000))       #座標(97,10+i)に5ケタのリバースクロー用の攻撃方向ベクトル(x軸)を書き込みます(小数点第3位まで行くので1000倍した値を書き込みます)
            func.write_system_data_num(self,103 -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_REVERSE_CLAW_SVY] * 1000))       #座標(103,10+i)に5ケタのリバースクロー用の攻撃方向ベクトル(y軸)を書き込みます(小数点第3位まで行くので1000倍した値を書き込みます)
            
            func.write_system_data_num(self,109 -1 +5,10+i,0,  5,int(self.replay_mode_stage_data[i][ST_CLAW_SHOT_SPEED] * 1000))        #座標(109,10+i)に5ケタのクローショットのスピードを書き込みます(小数点第3位まで行くので1000倍した値を書き込みます)
            func.write_system_data_num(self,115 -1 +4,10+i,0,  4,int(self.replay_mode_stage_data[i][ST_LS_SHIELD_HP]))           #座標(115,10+i)に4ケタのL'sシールドの耐久力を書き込みます
            
            func.write_system_data_num(self,144 -1 +3,10+i,0,  3,int(self.replay_mode_stage_data[i][ST_SHIP_ID]   ))          #座標(144,10+i)から3ケタの使用している機体のIDナンバーを書き込みます
            func.write_system_data_num(self,148 -1 +3,10+i,0,  3,int(self.replay_mode_stage_data[i][ST_SHIP_LEVEL]))          #座標(148,10+i)から3ケタの使用している機体のレベルを書き込みます
            func.write_system_data_num(self,152 -1 +4,10+i,0,  4,int(self.replay_mode_stage_data[i][ST_SHIP_EXP]  ))          #座標(152,10+i)から4ケタの使用している機体の経験値を書き込みます
            
            func.write_system_data_num(self,160 -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SLOT1])) #座標(160,10+i)から2ケタの使用している機体のメダルスロット1のメダルIDを書き込みます
            func.write_system_data_num(self,162 -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SLOT2])) #座標(162,10+i)から2ケタの使用している機体のメダルスロット2のメダルIDを書き込みます
            func.write_system_data_num(self,164 -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SLOT3])) #座標(164,10+i)から2ケタの使用している機体のメダルスロット3のメダルIDを書き込みます
            func.write_system_data_num(self,166 -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SLOT4])) #座標(166,10+i)から2ケタの使用している機体のメダルスロット4のメダルIDを書き込みます
            func.write_system_data_num(self,168 -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SLOT5])) #座標(168,10+i)から2ケタの使用している機体のメダルスロット5のメダルIDを書き込みます
            func.write_system_data_num(self,170 -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SLOT6])) #座標(170,10+i)から2ケタの使用している機体のメダルスロット6のメダルIDを書き込みます
            func.write_system_data_num(self,172 -1 +2,10+i,0,  2,int(self.replay_mode_stage_data[i][ST_SLOT7])) #座標(172,10+i)から2ケタの使用している機体のメダルスロット7のメダルIDを書き込みます
            
            func.write_system_data_num(self,176 -1 +3,10+i,0,  3,int(self.replay_mode_stage_data[i][ST_SCORE_STAR_MAG]))  #座標(176,10+i)から3ケタのスコアスター得点倍率を書き込み
            
            func.write_system_data_num(self,128 -1 +8,10+i,0,  8,int(self.replay_control_data_size[i])) #座標(128,10+i)に8ケタのコントロールパッド入力データが記録されたファイルのデータサイズを書き込みます
